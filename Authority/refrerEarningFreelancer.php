    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $('#show-table').DataTable({
                "pageLength" : 10,
                "ajax": {
                    url : '<?= site_url("Admin/DataTable")."?flag=freelancerEarning"; ?>',
                    type : 'GET'
                },
                "order": [[ 5, "desc" ]]
            });
            ...
            //         type: "POST",
            //         data: {data:data},
            //         url: "<?= site_url('getData').'?flag=area'; ?>",
            //         success: function(response) {
            //             var opt = "<select name='area' class='form-control'><option>Select Area</option>";
            //             var obj = jQuery.parseJSON(response);
            //             for (var i = 0; i < obj.length; i++) {
            //                 opt+= "<option>"+obj[i].area+"</option>";
            //             }
            //             $('#area-box').html(opt+"</select>");
            //         },
            //         error: function() {
            //             alert('Something went wrong.');
            //         }
            ...
            $('#btn-submit').click(function() {
                $.ajax({
                    ...
                    url: "<?= site_url('Admin/code').'?flag=agent-area'; ?>",
                    success: function(response) {
                        if (response == "success") {
	...
	<script>
		function remove(e) {
			$.ajax({
				type: "POST",
				data: {id:e.id},
				url: '<?= site_url("Admin/crud")."?flag=remove-agent"; ?>',
				success: function(response) {
					if (response == "success") {
		...
		function detail(e) {
			$.ajax({
				type: "POST",
				data: {id:e.id},
				url: '<?= site_url("Admin/getData")."?flag=agent"; ?>',
				success: function(response) {
					var obj = jQuery.parseJSON(response);
                    $('#id').val(obj.id);
					$('#user_id').html(obj.id);
					...
					$('#btn-login').attr('href', '<?= site_url('Admin/login').'?aid='; ?>'+obj.id);
                    $('#customer').modal('show');
        ...
        function hire() {
            ...
            $.ajax({
                type: "POST",
                data: {id:id},
                url: "<?= site_url('Admin/code').'?flag=approve'; ?>",
                success: function(response) {
                    if (response == "success") {
                        location.reload();
                    }
        ...
        function action(e) {
			$.ajax({
				type: "POST",
				data: {id:e.id},
				url: '<?= site_url("Admin/crud")."?flag=agent"; ?>',
				success: function(response) {
					if (response == "success") {
                        fire_toast('Data updated successfully.', 'success');
                        setTimeout(function() {
