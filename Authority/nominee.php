 <?php
    ...
    $nominee = $db->select_order('nominee',null,'date','desc');
    // print_r($nominee);
?>    
    <style>
        #show-table th {
    ...
    <div class="row">
        <div class="col-md-12 col-sm-12 col-xs-12">
            ...
            <div class="col-5 p-0"> 
                <!-- <input type="search" id="myInput" style="font-size: 14px;" class="cardborder"  placeholder="search">  -->
            </div>
                ...
                <table id="show-table" width="100%" class="table table-hover table-bordered" >
                    <thead>
                        <tr class="fixed-head">
                            ...
                            <th scope="col">Nominee Mobile No.</th>
                            <th scope="col">Nominee Age</th>
                            <th scope="col">Nominee Realtion</th>
                    ...
                    <tbody>
                        <?php $i=0; foreach ($nominee as $key ): $i++; 
                        $user = $db->select('users',['id'=>$key['id']]);
                        ...
                        <tr>
                            ...
                            <td><?= $key['age']; ?></td>
                            <td><?= $key['relation']; ?></td>
                            <td><?= $key['insurance_no']; ?></td>
                            <?php if ($key['status'] == "Pending"){ ?>
                            <td><button class="btn btn-danger"><?= $key['status']; ?></button></td>
                            <?php }else{ ?>
                            <td><button class="btn btn-success"><?= $key['status']; ?></button></td>
                            <?php } ?>
                            <td><a onclick='detail(this)' id="<?= $key['id']; ?>"><img src='http://kamgaro.com/bootstrap/image/detail-icon.png' class='cate-image' /></a> </td>
                            <td><a onclick='remove(this)' id="<?= $key['id']; ?>"><img src='http://kamgaro.com/bootstrap/image/delete-icon.png' class='cate-image' /></a></td>
                            <td><?= $key['date']; ?></td>
                        </tr>
                        <?php endforeach ?>
                    </tbody>
                                        ...
                                        <div class="col-lg-12 col-md-12 col-sm-12 group" style="text-align: left;">
                                            ...
                                            <div class="form-group">
                                                <form id="form-plan">
                                                    ...
                                                    <input class="form-control" type="text" id="id" name="insurence_no" placeholder="Insurance No" />
                                                    <p></p>
                                                    <input type="button" id="btn-update" class="btn btn-success" value="Approve">
                                                </form>
                                            </div>
                                            <br/>
                                        </div>
        ...
        $(document).ready(function() {
            ...
            $('#btn-update').click(function() {
                $.ajax({
                    ...
                    url: "<?= site_url('Admin/code').'?flag=approve_nominee'; ?>",
                    success: function(response) {
                        if (response == "success") {
                            fire_toast('Nominee Approved Successfully.', 'success');
                            setTimeout(() => {
                                location.reload();
                            }, 1000);
                        }
	...
	<script>
		function remove(e) {
			$.ajax({
				type: "POST",
				data: {id:e.id},
				url: '<?= site_url("Admin/crud")."?flag=remove-nominee"; ?>',
				success: function(response) {
					if (response == "success") {
						fire_toast("Nominee Removed successfully.", "success");
						setTimeout(function() {
							location.reload();
						}, 1000);
					}
		...
		function detail(e) {
			$.ajax({
				type: "POST",
				data: {id:e.id},
				url: '<?= site_url("Admin/getData")."?flag=users"; ?>',
				success: function(response) {
					...
					$('#btn-login').attr('href', '<?= site_url('Admin/login').'?id='; ?>'+obj.id);
                    $('#customer').modal('show');
				},
				error:function(){
					alert("Something went wrong.");
        ...
        function action(e) {
			$.ajax({
				type: "POST",
				data: {id:e.id},
				url: '<?= site_url("Admin/crud")."?flag=users"; ?>',
				success: function(response) {
					if (response == "success") {
