            ...
            <div class="x_panel">
                ...
                <div class="x_content div-table table-responsive">
                    ...
                    <table width="100%" class="table table-hover table-bordered">
                        ...
                        <tbody id="table">
                            <?php 
                                $result = $db->select('registration', null);
			
                                foreach($result as $row) :
                                    $value1 = $db->select('authority', ['user_id' => $_SESSION['authority'], 'authority' => 'Filter User']);
                            ...
                            <tr>
                                ...
                                <?php   } else {  ?>
                                 <td><a  target='_blank'><?= img(["../Customer/picture/".$row['profile'], 'cate-image']); ?></a></td>
                                 <?php } ?>
                                ...
                                <td><a><img src='https://kamgaro.com/bootstrap/image/detail-icon.png' class='cate-image' /></a></td>
                                <?php }  ?>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $('#show-table').DataTable({
                "pageLength" : 10,
                "ajax": {
                    url : '<?= site_url("Authority/DataTable")."?flag=customer"; ?>',
                    type : 'GET'
                },
            });
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $.ajax({
                            type: "POST",
                            data: {data:data},
                            url: "<?= site_url('Authority/filterData').'?flag=state-user'; ?>",
                            success: function(response) {
                                $('#table').html(response);
                            },
                            error: function() {
            ...
            $('#district').change(function() {
                ...
                $.ajax({
                    type: "POST",
                    data: {data:data},
                    url: "<?= site_url('Authority/filterData').'?flag=user-area'; ?>",
                    success: function(response) {
                        ...
                        $.ajax({
                            ...
                            url: "<?= site_url('Authority/filterData').'?flag=district-user'; ?>",
                            success: function(response) {
                                $('#table').html(response);
                            },
                            error: function() {
            ...
            $('#area').change(function() {
                ...
                $.ajax({
                    ...
                    url: "<?= site_url('Authority/filterData').'?flag=area-user'; ?>",
                    success: function(response) {
                        $('#table').html(response);
                    },
                    error: function() {
	...
	<script>
		function remove(e) {
			$.ajax({
				type: "POST",
				data: {id:e.id},
				url: '<?= site_url("Authority/crud")."?flag=remove-customer"; ?>',
				success: function(response) {
					if (response == "success") {
		...
		function detail(e) {
			$.ajax({
				type: "POST",
				data: {id:e.id},
				url: '<?= site_url("Authority/getData")."?flag=customer"; ?>',
				success: function(response) {
					var obj = jQuery.parseJSON(response);
        ...
        function action(e) {
			$.ajax({
				type: "POST",
				data: {id:e.id},
				url: '<?= site_url("Authority/crud")."?flag=customer"; ?>',
				success: function(response) {
					if (response == "success") {
